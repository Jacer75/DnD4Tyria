...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="/">
    <html lang="de">
      ...
      <body>
        <h1><xsl:value-of select="lore/@title"/></h1>

        <!-- Public -->
        <div>
          <xsl:copy-of select="lore/public/*"/>
        </div>
        ...
        <xsl:if test="lore/gm/*">
          <details>
            ...
            <div>
              <xsl:copy-of select="lore/gm/*"/>
            </div>
          </details>
        </xsl:if>

        <!-- Bilder: nur wenn vorhanden -->
        <xsl:if test="lore/images/*">
          <h2>Bilder</h2>
          <xsl:apply-templates select="lore/images"/>
        </xsl:if>

        <!-- Statblocks: nur wenn vorhanden -->
        <xsl:if test="lore/statblocks/*">
          <h2>Statblocks</h2>
          <xsl:apply-templates select="lore/statblocks/*"/>
        </xsl:if>
